...
## Project-Aware Assistant Rule (项目感知助手规则)

**此规则覆盖所有技术问题响应流程。**

### 触发条件
当用户消息包含以下技术关键词时，**禁止直接回答**，必须先执行项目确认：

**Cot项目关键词（方向1）**：
`PCS`、`MPPT`、`STS`、`柴发`、`柴油发电机`、`SOC`、`离网`、`并网`、`并柴发`、`防逆流`、`需量`、`直流母线`、`交流母线`、`储能`、`光伏`、`电池`、`EMS`、`能量管理`、`VF`、`PQ`、`ATS`、`切换`、`簇`、`均衡`、`滞环`、`充电`、`放电`、`逆变`、`整流`、`电压`、`电流`、`功率`、`拓扑`

**A股投资关键词（方向2）**：
`K线`、`均线`、`MACD`、`KDJ`、`RSI`、`BOLL`、`成交量`、`持仓`、`复盘`、`技术面`、`基本面`、`财报`、`F10`、`同花顺`、`KimiFinance`、`打新`、`IPO`、`中签`、`缴款`、`市值`、`涨跌幅`、`异动`、`预警`
...
用户回复数字/项目名称
        ↓
【自动执行元约定】
   方向1 (Cot): 读取 README → 02-Domain-Model.md 2.1节 → PENDING-ITEMS.md
   方向2 (A股): 读取 README → watchlist.json → portfolio.json
        ↓
【拓扑自检】心中默念功率流向
        ↓
正式回答用户问题
...
### 例外情况
- 用户明确指定项目（如"Cot项目的电压..."）→ 跳过询问，直接执行元约定
- 连续对话已确认项目 → 10分钟内无需重复确认
- 纯闲聊/非技术问题 → 正常响应

### 自检失败处理
如元约定文件不存在：
1. 报错并询问是否初始化项目
2. 提供选项：初始化 / 指定路径 / 跳过（不推荐）
