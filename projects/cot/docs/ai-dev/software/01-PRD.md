...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| 1.10 | 2026-05-09 | **修正**: FR-007 C2条件及C1/C3/C4/C5/C6通信条件 — 通信故障仅纳入关键设备（PCS/BAM/STS/电表），液冷机/除湿机/变压器温控仪通信故障走L3告警通道，不触发状态机跳转 |
...
**本项目采用 STS 快速切换方案**：电网异常时 STS 在电压跌落到 LVRT 触发阈值之前即切断，PCS 不承受故障电压，因此 PCS 无需 LVRT 能力。**注意**：若电网跌落速度过快，STS 切断前 PCS 可能先触发过流或欠压保护而停机。
...
| 条件 | 名称 | 详细定义 |
|------|------|----------|
| **C1** | 开机就绪 | 关键设备通讯正常 ∧ 所有设备无故障 ∧ 系统各功能节点无故障 ∧ PCS簇设备(主从、运行模式、运行状态=开机) ∧ 配置与状态正常 ∧ MPPT簇设备(主从、运行模式、运行状态) ∧ 配置与状态正常 ∧ BMS主从与配置数量匹配 ∧ 其他设备配置与参数正常 ∧ 直流侧电压参数满足离网供电高压(870V±压差) ∧ 交流母线电压参数满足离网供电(50Hz,电压相序正常) |
| **C2** | 故障条件 | 关键设备通讯不正常 ∨ 存在有故障设备 ∨ 存在故障的系统功能节点 |
| **C3** | 并市电条件 | 关键设备通讯正常 ∧ 所有设备无故障 ∧ 系统各功能节点无故障 ∧ ATS市电IO=1 ∧ STS并网状态位=1 ∧ 【可选】关口表市电电压参数正常(电压、相序、频率) ∧ **市电恢复后延时≥120s**（STS参数：STS需检测市电稳定120s后才允许重并） |
| **C4** | 并柴发条件 | 关键设备通讯正常 ∧ 所有设备无故障 ∧ 系统各功能节点无故障 ∧ ATS市电IO=0 ∧ 备用IO=1(柴发) ∧ STS并网状态位=1 ∧ 【可选】柴发侧电压参数正常(电压、相序、频率) |
| **C5** | 切离网条件 | 关键设备通讯正常 ∧ 所有设备无故障 ∧ 系统各功能节点无故障 ∧ STS并网状态位=0 ∧ 【可选】关口表市电电压异常(电压跌落<0.85Un或频率异常) |
| **C6** | 转待机条件 | 关键设备通讯正常 ∧ 所有设备无故障 ∧ 系统各功能节点无故障 ∧ PCS簇设备(运行状态=关机) ∧ 配置与状态正常 ∧ 直流侧电压参数满足待机高压(870V±压差) ∧ 交流母线电压参数正常(电压≈0,频率≈0) |

> **关键设备定义**：纳入C2通信故障判断的关键设备包括 PCS(1#~5#)、BAM(1~2)、STS、关口表、储能表、负载表、柴发表。液冷机、除湿机、变压器温控仪的通信故障为L3级一般告警，不触发C2状态跳转，亦不阻塞C1/C3/C4/C5/C6条件满足。
...
5. **PLL预同步**：并网期间PCS的PLL必须持续跟踪市电相位；离网切换时，PCS直接使用锁存相位角作为VF模式初始相位，无需重新锁相。这是20ms无缝切换的技术前提。
...
| 级别 | 名称 | 恢复方式 | 示例 |
|------|------|----------|------|
| **L1-致命** | 系统级停机故障 | **禁止自动恢复**，必须人工确认后复位 | 直流过压、绝缘故障、急停触发、STS硬件故障 |
...
| **E2xx** | BAM/BMS故障 | 单体过压、单体欠压、温差过大、SOC跳变、绝缘故障、通讯中断 |
...
| 指标 | 要求 | 说明 |
|------|------|------|
| **并离网切换时间** | **≤20ms（1个周波）** | 基于PCS PLL预同步无缝切换：STS切断瞬间PCS以锁存相位直接输出电压，负载断电间隙≈20ms |
| **功率控制精度** | **<±1%** | 稳态功率控制偏差（PCS可达标） |
| **充放电转换时间** | **<500ms** | 充电→放电、放电→充电模式切换 |
...
| 2 | ~~STS切换时间~~ | ~~负载供电连续性~~ | **已确认：≤20ms（1个周波），基于PCS PLL预同步无缝切换** |
