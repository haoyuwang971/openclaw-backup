...
| 版本 | 日期 | 变更内容 |
|------|------|---------|
| 1.21 | 2026-05-09 | **修正**: C2条件及C1/C3/C4/C5/C6通信条件 - 通信故障仅纳入关键设备（PCS/BAM/STS/电表），液冷机/除湿机/变压器温控仪通信故障走L3告警通道，不触发状态机跳转 |
...
| 条件 | 名称 | 逻辑表达式 | 关键参数 |
|------|------|------------|----------|
| **C1** | 开机就绪 | `CriticalCommOK ∧ AllNoFault ∧ SysNodesOK ∧ PCSAllOn ∧ PCSConfigOK ∧ MPPTConfigOK ∧ BMSMatch ∧ DCVoltOK(870V±Δ) ∧ ACVoltOK(50Hz)` | 关键通讯、故障、配置、电压、频率 |
| **C2** | 故障条件 | `CriticalCommFail ∨ AnyDeviceFault ∨ AnySysNodeFault` | 关键设备通讯中断、设备故障、功能节点故障 |
| **C3** | 并市电条件 | `CriticalCommOK ∧ AllNoFault ∧ ATS_Grid=1 ∧ STS_Close=1 ∧ 【可选】GridVoltOK` | ATS市电=1, STS合闸=1, 市电电压正常 |
| **C4** | 并柴发条件 | `CriticalCommOK ∧ AllNoFault ∧ ATS_Grid=0 ∧ ATS_Gen=1 ∧ STS_Close=1 ∧ GenVoltOK` | 市电=0, 备用=1, STS合闸=1, 柴发电压正常 |
| **C5** | 切离网条件 | `CriticalCommOK ∧ AllNoFault ∧ STS_Close=0 ∧ 【可选】GridVoltFail` | STS分闸=0, 市电/柴发异常 |
| **C6** | 转待机条件 | `CriticalCommOK ∧ AllNoFault ∧ PCS_AllOff ∧ DCVoltOK(870V±Δ) ∧ ACVoltZero` | PCS关机, 电压正常, 交流归零 |
...
bool CheckConditionC1(void) {
    return CheckCriticalCommOK() &&   // 仅检查关键设备通信（PCS/BAM/STS/电表）
           CheckAllNoFault() && 
           CheckSysNodesOK() &&
...
bool CheckConditionC3(void) {
    return CheckCriticalCommOK() &&   // 仅检查关键设备通信
           CheckAllNoFault() &&
           (DI_ATS_Grid == 1) &&
           (STS_GridStatus == 1) &&
...
**关键设备（Critical）通信故障 — 纳入C2状态机判断**：

| 设备类型 | 设备清单 | 通信总线 | C2影响 |
|---------|---------|---------|--------|
| PCS | 1#~5# | RS485-1/2 | **是** — 功率控制核心 |
| BAM | BAM1/BAM2 | 以太网TCP1/2 | **是** — SOC与电池保护核心 |
| STS | STS | RS485-3 | **是** — 并离网切换核心 |
| 关口表 | 安科瑞ADL400 | RS485-4 | **是** — 防逆流/需量核心 |
| 储能表 | 东鸿SDM630MCT-V2 | RS485-1/2 | **是** — 计量与功率方向 |
| 负载表 | 东鸿SDM630MCT-V2 | RS485-1/2 | **是** — 负载功率监测 |
| 柴发表 | 东鸿SDM630MCT-V2 | RS485-4 | **是** — 柴发并机状态判断 |

**非关键设备（Non-Critical）通信故障 — 走L3告警通道，不触发C2**：

| 设备类型 | 设备清单 | 通信总线 | 告警级别 |
|---------|---------|---------|---------|
| 液冷机 | 1#~4# | RS485-7 | **L3-一般告警** |
| 除湿机 | 1#~4# | RS485-6 | **L3-一般告警** |
| 变压器温控仪 | 1台 | RS485-8 | **L3-一般告警** |

**判定逻辑**：
```
CriticalCommOK  = PCS(1~5)通讯正常 ∧ BAM(1~2)通讯正常 ∧ STS通讯正常 
                    ∧ 关口表通讯正常 ∧ 储能表通讯正常 ∧ 负载表通讯正常 ∧ 柴发表通讯正常

CriticalCommFail = ¬CriticalCommOK  // 任一关键设备通信中断即触发

NonCriticalCommFail = 液冷机通信中断 ∨ 除湿机通信中断 ∨ 变压器温控仪通信中断
                      // 单独走L3告警，不进C2
```

**重要说明**：
1. 非关键设备通信中断时，状态机维持当前状态运行，不跳转至Fault态
2. 液冷机通信中断后，EMS通过BAM电池温度间接监控热管理状态（详见液冷机分级处理策略）
3. 若BAM温度因液冷机失效而异常升高，由BAM过温保护（地址43=5/6/7/8）触发设备故障，纳入`AnyDeviceFault`处理
...
```
C2 = CriticalCommFail ∨ AnyDeviceFault ∨ AnySysNodeFault
```

**关键设备通信故障定义**: 仅PCS、BAM、STS、电表（关口表/储能表/负载表/柴发表）的通信中断纳入C2。液冷机、除湿机、变压器温控仪通信中断走L3告警通道，不触发C2。
...
### 6.8 正常停机流程（Standby → SysStop）

**触发事件**: 本地HMI"关机"按钮 / 远程关机指令 / 系统正常停机条件满足

**执行顺序**（严格按步骤，每步确认后才进行下一步）：

```
Step 1: 停止策略计算
    └─ StrategyEngine停止运行，不再计算新的功率指令
    └─ 状态机进入停机子流程

Step 2: 功率归零（并网/离网/并柴发通用）
    └─ clearAllpowerSet()：所有PCS功率设定值设为0
    └─ 等待PCS实际功率归零（读取PCS_Power反馈，阈值<5kW）
    └─ 超时处理：若5s内未归零，上报PCS停机异常，转故障停机

Step 3: MPPT限功率归零
    └─ 所有MPPT功率百分比设为0（46110=0）
    └─ 等待MPPT输出侧功率归零（读取45218反馈）
    └─ 超时处理：若10s内未归零，上报MPPT停机异常

Step 4: 柴发停机（如运行中）
    └─ genStartStop(0)：DO-1断开，发出柴发停机信号
    └─ 等待柴发转速归零（监测DI柴发运行状态=0）
    └─ 超时处理：若120s内未停机，上报柴发停机超时，转故障停机

Step 5: STS分闸确认
    └─ EMS读取STS_Status状态位，确认STS已分闸
    └─ 若STS仍合闸，等待1s后再次确认（PCS功率归零后STS应自动分闸）
    └─ 超时处理：若3s内未分闸，上报STS分闸异常，转故障停机

Step 6: PCS关机序列（严格顺序）
    └─ 从机先关：PCS 2# → 3# → 4# → 5# 依次发关机指令（0x0291=0）
    └─ 主机最后关：PCS 1# 最后发关机指令
    └─ 每台关机后等待PCS_RunStatus=待机（0），间隔≥2s
    └─ 超时处理：单台PCS若5s内未进入待机，上报该PCS关机失败

Step 7: 数据保存与日志
    └─ 保存停机前系统运行数据快照（功率/SOC/状态/事件）
    └─ 记录正常停机日志，标注停机原因（人工/远程/自动）
    └─ 本地数据库执行CHECKPOINT，确保数据落盘

Step 8: 进入完全待机
    └─ 确认所有设备处于待机/关机状态
    └─ 系统状态机回到SysStandby初始态
    └─ 本地屏幕显示"系统已关机，可安全断电"
```

**异常分支**:
- 任何步骤超时或失败 → 转入 **6.9 故障停机流程**
- 急停信号触发 → 立即转入 **6.7 紧急停机**

**EMS可控 vs 设备自主**:
| 步骤 | EMS动作 | 设备自主 | 监测信号 |
|------|---------|----------|----------|
| 1 | ✅ 停止策略 | - | - |
| 2 | ✅ 功率清零 | ✅ PCS功率归零 | PCS_Power |
| 3 | ✅ MPPT限功率 | ✅ MPPT功率归零 | MPPT_OutPower |
| 4 | ✅ 柴发停机信号 | ✅ 柴发停机过程 | Gen_RunStatus |
| 5 | ✅ 读取确认 | ✅ STS自动分闸 | STS_Status |
| 6 | ✅ 下发关机指令 | ✅ PCS自主停机 | PCS_RunStatus |
| 7 | ✅ 数据保存 | - | - |

---

### 6.9 故障停机流程（任意状态 → SysFault → 安全停机）

**触发事件**: C2条件满足（L1/L2级故障）/ 正常停机步骤失败 / 保护动作触发

**与正常停机的核心区别**:
- 正常停机是**有序退出**，故障停机是**紧急保护**
- 故障停机不等待设备响应，以**最快速度**进入安全状态
- 故障停机后必须**人工确认**才能恢复
...
```
【P0-最高优先级：立即执行】
Step 1: 紧急功率清零（<100ms内完成）
    └─ clearAllpowerSet()：所有PCS功率设定值强制设为0
    └─ setMPPTLimit(0)：所有MPPT功率百分比强制设为0
    └─ 不等待PCS/MPPT反馈，立即执行下一步

【P1-高优先级：500ms内完成】
Step 2: 柴发停机（如运行中且故障非柴发本身）
    └─ genStartStop(0)：DO-1断开
    └─ 仅下发信号，不等待柴发实际停机

Step 3: 故障分级判定
    └─ 读取故障代码，判定L1/L2/L3级别
    └─ L1/L2：进入SysFault态，锁定状态机，禁止自动恢复
    └─ L3：记录告警，尝试降功率运行，不进入Fault态

【P2-中优先级：2s内完成】
Step 4: 状态机锁定
    └─ 系统状态强制切换至 SysFault
    └─ 状态机停止扫描，冻结当前状态
    └─ 禁止任何自动状态转换（包括并网/离网切换）

Step 5: 告警与通知
    └─ 本地声光告警（DO-3故障灯输出）
    └─ 云端MQTT告警推送（含故障代码/时间/设备ID）
    └─ 本地屏幕弹窗显示故障详情
    └─ 若L1级故障，触发蜂鸣器持续鸣响

【P3-低优先级：5s内完成】
Step 6: 数据保全
    └─ 保存故障前1秒的运行数据快照（用于事后分析）
    └─ 记录故障发生时的完整系统状态
    └─ 写保护关键数据区域，防止后续操作覆盖

Step 7: 设备状态监测（持续进行）
    └─ 持续读取各设备RunStatus，确认是否已安全停机
    └─ 监测STS_Status、PCS_RunStatus、Gen_RunStatus
    └─ 若设备未按要求停机，记录"设备停机异常"次级故障
```
...
【人工恢复确认流程】
1. 运维人员现场/远程检查故障原因
2. 排除故障源（修复设备/恢复通信/清除告警）
3. 本地HMI或Web点击"故障恢复确认"
4. EMS验证AllNoFault条件满足
5. 状态机从SysFault → SysStandby（需C6条件）
6. 系统重新进入待机，等待开机指令
```
...
| 故障源 | 停机重点 | 特殊处理 |
|--------|----------|----------|
| **PCS故障** | 功率清零为主 | 主机PCS故障时，从机PCS同步停机 |
| **BMS故障** | 立即停止充放电 | 禁止任何PCS功率指令，进入Fault态 |
| **MPPT故障** | 限功率归零 | 单路MPPT故障不影响其他路运行（L3级） |
| **STS故障** | 立即切离网 | STS硬件故障时，依赖PCS自主保护分闸 |
| **通信故障** | 冻结功率 | 通信中断时保持当前功率（不增加也不减少），等待恢复 |
| **防逆流触发** | 功率清零+切离网 | 属于保护动作，60s持续逆流进入Fault态 |
| **柴发故障** | 柴发停机+切离网 | 柴发故障时STS应自动切离网，PCS转VF |

**EMS可控 vs 设备自主**:
| 步骤 | EMS动作 | 设备自主 | 监测信号 |
|------|---------|----------|----------|
| 1 | ✅ 紧急功率清零 | - | - |
| 2 | ✅ 柴发停机信号 | ✅ 柴发保护停机 | Gen_RunStatus |
| 3 | ✅ 故障分级判定 | - | Fault_Code |
| 4 | ✅ 状态机锁定 | - | - |
| 5 | ✅ 告警通知 | - | - |
| 6 | ✅ 数据保全 | - | - |
| 7 | ✅ 持续监测 | ✅ 设备自主保护 | STS/PCS/Gen_Status |

**重要说明**: 故障停机时EMS以"软停机+状态锁定"为主，PCS和STS的物理保护动作（如硬件过流保护、自动分闸）由设备自主完成，EMS负责监测和记录。
